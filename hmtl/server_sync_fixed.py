...
class DataSyncHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        elif self.path.startswith('/api/doPost'):
            self.handle_doPost()
        else:
    ...
    def handle_doPost(self):
        try:
            print("Recebendo dados via doPost...")
            ...
            # Salvar dados localmente (simulando Google Sheets)
            # Aqui você pode adicionar lógica para salvar em JSON ou processar os dados
            ...
            self.send_header('Access-Control-Allow-Origin', '*')
            self.end_headers()
            self.wfile.write(json.dumps({"status": "success", "message": "Dados recebidos com sucesso"}).encode())

        except Exception as e:
            print(f"Erro no doPost: {e}")
            self.send_error(500, f"Erro interno: {str(e)}")
